...
void pcm1774_VolumeAdjust(int i)
{
	//read current volume and adjust by i
	unsigned char volumeLeft = pcm1774_readByte(PCM1774_HPA_L) + ( (i*190)/300 );
	unsigned char volumeRight = pcm1774_readByte(PCM1774_HPA_R) + ( (i*190)/300 );

	//check for overflow caused by bouncing switch
	if (i > 0)
	{
		if ((volumeLeft > 63) || (volumeRight > 63))
		{
			volumeLeft = volumeRight = 63;
		}
	}
	if (i < 0)
	{
	...
	pcm1774_sendByte(PCM1774_HPA_L, volumeLeft);
	pcm1774_sendByte(PCM1774_HPA_R, volumeRight);
}

unsigned char pcm1774_GetVolume(void)
{
	unsigned char volumeLeft = pcm1774_readByte(PCM1774_HPA_L);
	unsigned char volumeRight = pcm1774_readByte(PCM1774_HPA_R);
