...
PRIVILEGED_DATA static portTickType xNextTaskUnblockTime						= ( portTickType ) portMAX_DELAY;
	...
	static void prvGenerateRunTimeStatsForTasksInList( const char *pcWriteBuffer, xList *pxList, unsigned long ulTotalRunTime, char resetStats ) PRIVILEGED_FUNCTION;
#endif
 ...
 */
	...
	static void prvListTaskWithinSingleList( const char *pcWriteBuffer, xList *pxList, signed char cStatus ) PRIVILEGED_FUNCTION;
...
/*-----------------------------------------------------------*/
	...
	unsigned portBASE_TYPE uxQueue;
		...
		vTaskSuspendAll();
		{
			...
			strcpy(pcWriteBuffer, "   Task Name ST Pr StkFree T#\r\n");
...
/*----------------------------------------------------------*/
	...
	void vTaskGetRunTimeStats( char *pcWriteBuffer, char resetStats)
	{
		unsigned portBASE_TYPE uxQueue;
		...
		vTaskSuspendAll();
		{
			...
			*pcWriteBuffer = '\0';
			...
			do
			{
				...
				if( listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) == pdFALSE )
				{
					prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), ulTotalRunTime, resetStats );
				}
			}while( uxQueue > ( unsigned short ) tskIDLE_PRIORITY );

			if( listLIST_IS_EMPTY( pxDelayedTaskList ) == pdFALSE )
			{
				prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, ( xList * ) pxDelayedTaskList, ulTotalRunTime, resetStats );
			}

			if( listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) == pdFALSE )
			{
				prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, ( xList * ) pxOverflowDelayedTaskList, ulTotalRunTime, resetStats );
			}
			...
			#if ( INCLUDE_vTaskDelete == 1 )
			{
				if( listLIST_IS_EMPTY( &xTasksWaitingTermination ) == pdFALSE )
				{
					prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, &xTasksWaitingTermination, ulTotalRunTime, resetStats);
				}
			}
			#endif
			...
			#if ( INCLUDE_vTaskSuspend == 1 )
			{
				if( listLIST_IS_EMPTY( &xSuspendedTaskList ) == pdFALSE )
				{
					prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, &xSuspendedTaskList, ulTotalRunTime, resetStats );
				}
			}
#endif

			unsigned int overheadPercent = ulISROverhead/ulTotalRunTime ;
			if(overheadPercent < 1) {
				siprintf( (char*)pcWriteBuffer + strlen(pcWriteBuffer), ( char * ) "%8s  %10u   <1%%\r\n", "<OVERHEAD>", (unsigned int)ulISROverhead);
			}
			else {
				siprintf( (char*)pcWriteBuffer + strlen(pcWriteBuffer), ( char * ) "%8s  %10u  %3u%%\r\n", "<OVERHEAD>", (unsigned int)ulISROverhead, overheadPercent);
			}
...
/*-----------------------------------------------------------*/
	...
	static void prvListTaskWithinSingleList( const char *pcWriteBuffer, xList *pxList, signed char cStatus )
	{
		...
		do
		{
			...
			siprintf( (char*)pcWriteBuffer + strlen(pcWriteBuffer),
					(char*) "%8s  %c  %u  %5u  %u\r\n",
...
/*-----------------------------------------------------------*/
	...
	static void prvGenerateRunTimeStatsForTasksInList( const char *pcWriteBuffer, xList *pxList, unsigned long ulTotalRunTime, char resetStats )
	{
	volatile tskTCB *pxNextTCB, *pxFirstTCB;
		...
		do
		{
			...
			if( ulTotalRunTime > 0UL )
			{
				...
				if( pxNextTCB->ulRunTimeCounter == 0UL )
				{
					...
					siprintf( (char*) pcWriteBuffer + strlen(pcWriteBuffer), (char*) "%8s  %10u    0%%\r\n", pxNextTCB->pcTaskName, 0 );
				}
				else
				{
...
/*-----------------------------------------------------------*/
	...
	register unsigned short usCount = 0U;
		...
		// @return Stack size in bytes, so remove this line:
