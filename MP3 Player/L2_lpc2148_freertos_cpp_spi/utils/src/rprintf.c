...
// 
// To do: %f, %F for floating point numbers
//
...
static void rprintf_putCharSource(unsigned char c)
{
	if(c == '\n') putcharfunc('\r', 99999);
	putcharfunc(c, 99999);
}
...
// #warning "use long"
  ...
  for (;;){
    while ((format_flag = *(format++)) != '%'){      // Until '%' or '\0' 
      ...
      rprintf_putCharSource(format_flag);
    }
    ...
    case 's':
      ...
      while(*ptr) { rprintf_putCharSource(*ptr); ptr++; }
      continue;
#endif
        ...
        if(s_val < 0) //Value negativ ?
         {
          ...
          rprintf_putCharSource('-');    //Output sign
         }
